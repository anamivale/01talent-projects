...
func FindPaths(colony *structs.AntColony) ([]structs.Path, map[int][]int, int) {
	var allPaths []structs.Path
	var queue [][]string
	...
	for len(queue) > 0 {
		...
		if currentRoom == colony.End {
			allPaths = append(allPaths, structs.Path{Rooms: currentPath})
			continue
		}
...
func ChooseOptimumPath(paths []structs.Path, Antcolony *structs.AntColony) ([]structs.Path, map[int][]int, int) {
...
func OptimizedPaths1(paths []structs.Path) []structs.Path {
	optimized := make([]structs.Path, 0)
...
func Check(path []string, optimized []structs.Path) bool {
...
func OptimizedPaths2(paths []structs.Path, colony *structs.AntColony) []structs.Path {
	half := colony.Ants / 2
	optimized := make([]structs.Path, 0)
	optimized = append(optimized, paths[0])
	for i := 1; i < len(paths); i++ {
...
func Check2(path []string, optimized []structs.Path) (bool, int) {
...
func Remove(optimized []structs.Path, index int) []structs.Path {
	new := make([]structs.Path, 0)
